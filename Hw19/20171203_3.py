...
def updateWeights(Weights,gradients):
    global X,Y,alpha,m,param
    W1 = Weights['W1']
    W2 = Weights['W2']
    W3 = Weights['W3']
    W4 = Weights['W4']
    
    W1 = W1 - alpha * gradients["dW1"] - param*W1
    W2 = W2 - alpha * gradients["dW2"] - param*W2
    W3 = W3 - alpha * gradients["dW3"] - param*W3
    W4 = W4 - alpha * gradients["dW4"]- param*W4
...
def Model(losstype,epochs):
    ...
    for i in range(0,epochs+1):
        ...
        accr.append(checkaccuracy(Weights))

    return Weights,utils,accr
def checkaccuracy(Weights):
    ...
    for i in range(len(A4)):
        if(A4[i] <.5 and test_labels[i]==0):
            ct+=1
        elif(A4[i] >=.5 and test_labels[i]==1):
            ct+=1
    return (ct*100/len(A4))

def main():
    global X,Y,hid_1,hid_2,hid_3,out,alpha,epochs
    final,cost,accr = Model(0,epochs)
    final_mse,costmse,accr = Model(0,epochs)
    # checkaccuracy(final_mse)
    # print(accr)
    plt.plot([i for i in range(epochs+1)],accr)
    plt.title("MSE Accuracy vs Epochs L0 Regularised")
