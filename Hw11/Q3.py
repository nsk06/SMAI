...
# gdpca(data_mean)
def difffunction_l1(X,C):
    reconstruct = np.matmul(X,w)
    reconstruct = np.matmul(reconstruct,np.transpose(w))
    e1 = np.matmul(X.T,reconstruct)
    e2 = (e1+e1.T)
    e2 = np.matmul(e2,w)/LA.norm(reconstruct)
    total = e2+ 5/1000*(X.T@np.sign(np.matmul(X,w))+np.sign(w))
    return total
...
def gdpcal1(X):
    ...
    iters = 150
    for i in range(iters):
        w = w+alpha*difffunction_l1(X,w)
    project = np.matmul(X,w)
    plt.scatter(-project[:,0:1],-project[:,1:2])
    plt.title("Gradient descent Pca L1")
    plt.show()

def difffunction_l2(X,w):
    reconstruct = np.matmul(X,w)
    reconstruct = np.matmul(reconstruct,np.transpose(w))
    e1 = np.matmul(X.T,reconstruct)
    e2 = (e1+e1.T)
    e2 = np.matmul(e2,w)/LA.norm(reconstruct)
    total = e2+ ((5/1000)**2)*((X.T@X@w)/LA.norm(np.matmul(X,w))+ w/LA.norm(w))
    return total
...
def gdpcal2(X):
    ...
    iters = 150
    for i in range(iters):
        w = w+alpha*difffunction_l2(X,w)
    project = np.matmul(X,w)
    plt.scatter(-project[:,0:1],-project[:,1:2])
    plt.title("Gradient descent Pca L2")
    plt.show()
